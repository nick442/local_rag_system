...
class PipelineBenchmarker:
        ...
        for config in worker_configs:
            ...
            try:
                ...
                config_results[config_name] = {
                    ...
                    'avg_time_per_file': total_time / result.files_processed if result.files_processed > 0 else 0
                }
                
                print(f"  Processed {result.files_processed} files in {total_time:.2f}s "
                      f"({config_results[config_name]['files_per_second']:.2f} files/sec)")
    ...
    async def run_full_benchmark(self) -> Dict[str, Any]:
        """Run all component benchmarks and compile results."""
        print("\n=== STARTING FULL PIPELINE BENCHMARK ===")
        
        # Validate test directory
        if not self.test_dir.exists():
            raise FileNotFoundError(f"Test directory not found: {self.test_dir}")
        
        print(f"Using test directory: {self.test_dir} ({len(list(self.test_dir.glob('*.txt')))} files)")
