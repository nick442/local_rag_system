...
# Add project root to path (so imports find src/)
...
class RAGBenchmark:
    ...
    def benchmark_generation_speed(self, num_tests: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(num_tests):
                ...
                # Non-streaming generation for tokens per second
                start_gen = time.time()
                result = wrapper.generate(prompt, max_tokens=100, temperature=0.7)
                end_gen = time.time()
                
                generated_time = end_gen - start_gen
                generated_tokens = len(result.split())
                
                generation_times.append(generated_time)
                first_token_latencies.append(first_token_latency)
                tokens_per_second_list.append(generated_tokens / generated_time if generated_time > 0 else 0)
            ...
            print(f"✅ Avg gen time: {results['avg_generation_time']:.2f}s, "
                  f"First token: {results['avg_first_token_latency_ms']:.1f}ms, "
                  f"Throughput: {results['avg_tokens_per_second']:.1f} tok/s")
    ...
    def benchmark_rag_pipeline(self, num_tests: int = 5) -> Dict[str, Any]:
        """Benchmark end-to-end RAG pipeline performance."""
        print(f"\n🔄 Benchmarking RAG pipeline ({num_tests} tests)...")
        
        try:
            # Initialize RAG pipeline with config settings
            model_path = self.config.get('model', {}).get('path')
            embedding_path = self.config.get('embedding', {}).get('path')
            db_path = self.config.get('database', {}).get('path', 'data/rag_vectors.db')
            
            if not all([model_path, embedding_path]):
                return {'error': 'Missing model or embedding path in config'}
            ...
            for i in range(num_tests):
                ...
                # Measure retrieval time by tapping into retriever
                retrieval_start = time.time()
                results = rag.retriever.retrieve(query, k=5)
                retrieval_end = time.time()
                
                # Measure end-to-end time via pipeline query
                response = rag.query(query, k=5)
                end_time = time.time()
                
                # Calculate metrics
                query_times.append(end_time - start_time)
                retrieval_times.append(retrieval_end - retrieval_start)
                
                response_text = response.get('response', '') or response.get('answer', '')
                response_qualities.append(len(response_text))  # proxy metric
            
            results = {
                'avg_end_to_end_time': statistics.mean(query_times),
                'avg_retrieval_time': statistics.mean(retrieval_times),
                'avg_response_length': statistics.mean(response_qualities),
                'success': True
            }
            
            print(f"✅ Avg E2E: {results['avg_end_to_end_time']:.2f}s, "
